...
|-----------|---------------|-------|
| `get_content()` | `get_page_with_adf()` | v2 returns ADF content |
| `create_content()` | `create_page_with_adf()` | v2 accepts ADF content |
| `update_content()` | `update_page_with_adf()` | v2 accepts ADF content |
| `search_content()` | `search_pages_with_cursor()` | v2 uses cursor pagination |

### Pagination

| v1 Approach | v2 Approach | Benefits |
|-------------|-------------|----------|
| `limit=50, start=100` | `limit=50, cursor="token"` | Better performance, no offset limits |
| Offset-based | Cursor-based | Consistent results, handles large datasets |
